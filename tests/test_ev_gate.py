...
class TestExpectedValueGate:
    def test_breakeven_is_negative_with_fees(self):
        ...
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.5, current_contract_price_cents=50,
            exchange_fee_pct=0.07, minimum_ev_cents=0.0
        )
        assert result is False

    def test_strong_edge_is_profitable(self):
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.9, current_contract_price_cents=10,
            exchange_fee_pct=0.07, minimum_ev_cents=10.0
        )
        assert result is True

    def test_zero_probability_never_profitable(self):
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.0, current_contract_price_cents=50,
            exchange_fee_pct=0.07, minimum_ev_cents=0.0
        )
        assert result is False
    ...
    def test_100_pct_probability_always_profitable(self):
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=1.0, current_contract_price_cents=10,
            exchange_fee_pct=0.07, minimum_ev_cents=0.0
        )
        assert result is True
    ...
    def test_high_fee_kills_marginal_edge(self):
        ...
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.55, current_contract_price_cents=45,
            exchange_fee_pct=0.50, minimum_ev_cents=0.0
        )
        assert result is False
    ...
    def test_minimum_ev_threshold_enforced(self):
        ...
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.6, current_contract_price_cents=40,
            exchange_fee_pct=0.07, minimum_ev_cents=50.0
        )
        assert result is False
    ...
    def test_negative_ev_barrier_allows_suboptimal_edges(self):
        ...
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.5, current_contract_price_cents=50,
            exchange_fee_pct=0.07, minimum_ev_cents=-2.0
        )
        assert result is True
    ...
    def test_negative_ev_barrier_still_blocks_worse_edges(self):
        ...
        result = ExpectedValueGate.is_profitable_after_fees(
            predicted_win_prob=0.5, current_contract_price_cents=50,
            exchange_fee_pct=0.07, minimum_ev_cents=-1.0
        )
        assert result is False
